...
ALTER TABLE `user`
  ADD COLUMN `nick`  VARCHAR(50)  DEFAULT ''  COMMENT '昵称'         AFTER `pwd`,
  ADD COLUMN `desc`  VARCHAR(255) DEFAULT ''  COMMENT '个性签名/简介' AFTER `nick`,
  ADD COLUMN `sex`   INT UNSIGNED DEFAULT 0   COMMENT '性别: 0=未知, 1=男, 2=女' AFTER `desc`,
  ADD COLUMN `icon`  VARCHAR(500) DEFAULT ''  COMMENT '头像URL'      AFTER `sex`,
  ADD COLUMN `back`  VARCHAR(500) DEFAULT ''  COMMENT '背景图URL'    AFTER `icon`;
```
...
- ```c++
  ...
  ```
   ...
   [循环 count 次]
       ...
       sex: uint32_t
       icon: string
       status: uint32_t (0=待处理, 1=已同意, 2=已拒绝)